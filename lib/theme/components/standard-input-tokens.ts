//. 📍 lib/theme/components/standard-input-tokens.ts
...
export interface StandardInputVariantTokens {
	background: string;
	border: string;
	text: string;
	placeholder: string;
...
export interface StandardInputSizeTokens {
	height: string;
	fontSize: string;
	paddingX: string;
	paddingY: string;
...
export type StandardInputTokens = {
	variants: Record<StandardInputVariant, StandardInputVariantTokens>;
	sizes: Record<StandardInputSize, StandardInputSizeTokens>;
};
//#endregion ![def]

//#region [main] - 🔧 LOGIC 🔧
export function generateStandardInputTokens(
	...
	//> 📝 Definiciones comunes para estados (deshabilitado, solo lectura)
	...
	//> 📝 Definiciones comunes para estados de validación (error, éxito)
	...
	//> 💡 La variante 'default' es especial: borde neutral, pero foco primario.
	const defaultVariantNormalBackground = neutral.bg;
	const defaultVariantIsEditingBackground = tinycolor(tertiary.bg)
		.setAlpha(0.8)
		.toString();

	const defaultVariantTokens: StandardInputVariantTokens = {
