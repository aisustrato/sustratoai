...
function convertirNodoContenido(
  ...
  switch (node.type) {
    ...
    case "blockquote": {
      ...
      // Usamos el MISMO generador del padre para que los IDs sigan la
      // numeración de la sección y no colisionen con sus hermanos
      // (un generador con contadores reseteados producía IDs duplicados,
      // p.ej. dos "h1_0.h2_2.p_0" → key duplicada en React).
      const hijos: NodoHoja[] = [];
      for (const child of node.children) {
        const hoja = convertirNodoContenido(child, generador);
