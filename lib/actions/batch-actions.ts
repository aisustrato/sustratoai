...
function segmentArticles(
	...
	if (totalItems === 0 || value <= 0)
		return { numBatches: 0, articlesPerBatchArray: [] };
	let numBatches =
		mode === "batches"
			? Math.min(value, totalItems)
	...
	return {
		numBatches,
		articlesPerBatchArray: Array.from(
			{ length: numBatches },
...
async function verificarPermisoBatchActions(
	...
	if (rpcError) {
		console.error(
			`[AUTH_CHECK_ERROR] RPC has_permission_in_project (${permission}):`,
			rpcError.message
		);
		return false;
	}
...
export async function simulateBatches(
	...
	console.log(
		`🧪 [${opId}] Iniciando simulateBatches (Vista, Asig.Bloques) para proyecto: ${projectId}`
	);
	...
	if (!projectId || value <= 0) {
		return {
			success: false,
			error: "Payload inválido (projectId o value).",
			errorCode: "INVALID_PAYLOAD_BASIC",
		};
	}
	...
	if (
		...
		return {
			success: false,
			error: "Payload inválido: Se requiere al menos un miembro seleccionado.",
			errorCode: "INVALID_PAYLOAD_MEMBERS",
		};
	}
	...
	try {
		...
		const tienePermiso = await verificarPermisoBatchActions(
			supabase,
			currentUser.id,
			projectId
		);
		if (!tienePermiso) {
			return {
				success: false,
				error: "No tienes permiso para simular/crear lotes en este proyecto.",
				errorCode: "FORBIDDEN_SIMULATE",
			};
		}
		...
		if (countError) {
			console.error(
				`❌ [${opId}] Error contando desde la VISTA:`,
				JSON.stringify(countError, null, 2)
			);
			return {
				success: false,
				error: `Error al contar artículos desde la vista (${countError.message})`,
				errorCode: "DB_VIEW_COUNT_ERROR",
			};
		}
		...
		if (totalEligibleArticles === null) {
			...
			return {
				success: false,
				error: "El conteo de artículos (vista) retornó NULL.",
				errorCode: "DB_VIEW_COUNT_NULL",
			};
		}
		console.log(
			`[${opId}] Artículos elegibles encontrados (desde Vista para SIMULACIÓN): ${totalEligibleArticles}`
		);
		...
		if (totalEligibleArticles === 0) {
			return {
				...
				data: {
					distribution: [],
					totalBatchesCalculated: 0,
					totalEligibleArticles: 0,
					articlesPerMember: {},
				},
			};
		}

		const { numBatches, articlesPerBatchArray } = segmentArticles(
		...
		if (numBatches > 2000) {
			return {
				success: false,
				error: "La simulación excede el máximo de 2000 lotes (Vista).",
				errorCode: "TOO_MANY_BATCHES_VIEW",
			};
		}
		...
		let targetBatchesForThisMemberBlock =
			baseBatchesPerMember +
			(currentMemberArrIndex < remainingBatchesExtra ? 1 : 0);

		for (let i = 0; i < numBatches; i++) {
			if (
				batchesAssignedToThisMemberBlock >= targetBatchesForThisMemberBlock &&
				currentMemberArrIndex < numSelectedMembers - 1
			) {
				currentMemberArrIndex++;
				batchesAssignedToThisMemberBlock = 0;
				targetBatchesForThisMemberBlock =
					baseBatchesPerMember +
					(currentMemberArrIndex < remainingBatchesExtra ? 1 : 0);
			}
			...
			distribution.push({
				batchNumberObjective: i + 1,
				articleCount: articleCountForThisBatch,
				assignedToMemberId: assignedMemberId,
			});
			if (assignedMemberId) {
				articlesPerMemberCounter[assignedMemberId] =
					(articlesPerMemberCounter[assignedMemberId] || 0) +
					articleCountForThisBatch;
			}
			batchesAssignedToThisMemberBlock++;
		}

		console.log(
			`🎉 [${opId}] Simulación (Vista, Asig. Bloques) completada. ${distribution.length} lotes.`
		);
		return {
			success: true,
			data: {
				distribution,
				totalBatchesCalculated: numBatches,
				totalEligibleArticles,
				articlesPerMember: articlesPerMemberCounter,
			},
		};
	} catch (error) {
		...
		return {
			...
			errorCode: "INTERNAL_SERVER_ERROR_SIM",
		};
	}
}
...
export async function createBatches(
	...
	console.log(
		`🚀 [${opId}] Iniciando createBatches (CHUNKED) para proyecto: ${projectId}`
	);
	...
	if (!projectId || value <= 0) {
		return {
			success: false,
			error: "Payload inválido (projectId o value).",
			errorCode: "INVALID_PAYLOAD_BASIC_CREATE",
		};
	}
	if (mode === "size" && (value < MIN_BATCH_SIZE || value > MAX_BATCH_SIZE)) {
		return {
			success: false,
			error: `El tamaño del lote debe estar entre ${MIN_BATCH_SIZE} y ${MAX_BATCH_SIZE}.`,
			errorCode: "INVALID_BATCH_SIZE_RANGE_CREATE",
		};
	}
	if (
		!selectedMemberIds ||
		...
		return {
			success: false,
			error: "Payload inválido: Se requiere al menos un miembro seleccionado.",
			errorCode: "INVALID_PAYLOAD_MEMBERS_CREATE",
		};
	}
	...
	try {
		...
		if (!currentUser) {
			return {
				success: false,
				error: "Usuario no autenticado.",
				errorCode: "UNAUTHENTICATED_CREATE",
			};
		}
		const tienePermiso = await verificarPermisoBatchActions(
			supabase,
		...
		if (!tienePermiso) {
			return {
				success: false,
				error: "No tienes permiso para crear lotes en este proyecto.",
				errorCode: "FORBIDDEN_CREATE",
			};
		}

		console.log(
			`[${opId}] Obteniendo TODOS los IDs de artículos elegibles desde la VISTA (en chunks)...`
		);
		...
		while (moreDataAvailable) {
			...
			if (chunkError) {
				console.error(
					`❌ [${opId}] Error obteniendo chunk de IDs elegibles:`,
					JSON.stringify(chunkError, null, 2)
				);
				return {
					success: false,
					error: `Error al obtener IDs elegibles (chunk ${currentOffset}): ${chunkError.message}`,
					errorCode: "DB_VIEW_FETCH_CHUNK_ERROR",
				};
			}

			if (chunkData && chunkData.length > 0) {
				const idsInChunk = chunkData
					.map((a) => a.article_id)
			...
			console.log(
				`[${opId}]   Chunk fetched. Total IDs acumulados: ${allEligibleArticleIds.length}`
			);
		}
		...
		console.log(
			`[${opId}] IDs de artículos elegibles TOTALES obtenidos para CREACIÓN: ${totalEligibleArticles}`
		);

		if (totalEligibleArticles === 0) {
			return {
				success: false,
				error: "No hay artículos elegibles para lotear.",
				errorCode: "NO_ELIGIBLE_ARTICLES_CREATE_FINAL",
			};
		}
		...
		if (numBatches === 0) {
			return {
				success: false,
				error: "No se pueden crear 0 lotes.",
				errorCode: "ZERO_BATCHES_TO_CREATE",
			};
		}
		if (numBatches > 2000) {
			return {
				success: false,
				error: "La creación excede el máximo de 2000 lotes.",
				errorCode: "TOO_MANY_BATCHES_CREATE",
			};
		}
		...
		if (maxBatchNumError && maxBatchNumError.code !== "PGRST116") {
			console.error(
				`❌ [${opId}] Error obteniendo max batch_number:`,
				maxBatchNumError
			);
			return {
				success: false,
				error: `Error determinando número de lote: ${maxBatchNumError.message}`,
				errorCode: "DB_MAX_BATCH_NUM_ERROR",
			};
		}
		...
		for (let i = 0; i < numBatches; i++) {
			...
			const assignedMemberId = selectedMemberIds[currentMemberArrIndex];
			const articleCountForThisBatch = articlesPerBatchArray[i];
			const batchName = batchNamePrefix
				? `${batchNamePrefix}${assignedBatchNumber}`
				: `Lote ${assignedBatchNumber} (${projectId.substring(0, 4)}...)`;
			...
			if (batchInsertError || !newBatch) {
				console.error(
					`❌ [${opId}] Error insertando en article_batches:`,
					batchInsertError
				);
				return {
					success: false,
					error: `Error al crear el lote ${assignedBatchNumber}: ${
						batchInsertError?.message || "No se retornó ID."
					}`,
			...
			const itemsToInsert: Database["public"]["Tables"]["article_batch_items"]["Insert"][] =
				[];
			const articlesForThisBatch = availableArticleIds.slice(
				currentArticlePoolIndex,
				currentArticlePoolIndex + articleCountForThisBatch
			);
			for (const articleId of articlesForThisBatch) {
				if (!articleId) {
					console.warn(`[${opId}] articleId nulo/undefined saltado.`);
					continue;
				}
				itemsToInsert.push({
			...
			if (itemsToInsert.length > 0) {
				...
				if (itemsInsertError) {
					console.error(
						`❌ [${opId}] Error insertando items para batch ${newBatch.id}:`,
						itemsInsertError
					);
					return {
						success: false,
						error: `Error al agregar artículos al lote ${assignedBatchNumber}: ${itemsInsertError.message}`,
						errorCode: "DB_BATCH_ITEMS_INSERT_ERROR",
					};
				}
		...
		console.log(
			`✅ [${opId}] Creación de lotes completada. Lotes creados: ${createdBatchIds.length}, Items totales: ${totalItemsAssignedToBatches}`
		);
		return {
			success: true,
			data: {
				createdBatchesCount: createdBatchIds.length,
				totalItemsCreated: totalItemsAssignedToBatches,
		...
		return {
			success: false,
			error: `Error interno del servidor: ${(error as Error).message}`,
			errorCode: "INTERNAL_SERVER_ERROR_CREATE",
		};
	}
}
...
export async function resetProjectBatchesIfNotInitialized(
	...
	console.log(
		`🗑️ [${opId}] Iniciando resetProjectBatches para proyecto: ${projectId}`
	);
	...
	if (!projectId) {
		return {
			...
			errorCode: "INVALID_PROJECT_ID_RESET",
		};
	}
	...
	try {
		...
		if (!currentUser) {
			return {
				success: false,
				error: "Usuario no autenticado.",
				errorCode: "UNAUTHENTICATED_RESET",
			};
		}
		const tienePermiso = await verificarPermisoBatchActions(
			supabase,
			currentUser.id,
			projectId,
			PERMISO_GESTIONAR_LOTES_PARA_BORRADO
		);
		if (!tienePermiso) {
			return {
				success: false,
				error:
					"No tienes permiso para eliminar/resetear lotes en este proyecto.",
				errorCode: "FORBIDDEN_RESET",
			};
		}
		...
		if (checkError) {
			return {
				success: false,
				error: `Error al verificar estado de los lotes: ${checkError.message}`,
				errorCode: "DB_CHECK_INIT_ERROR",
			};
		}
		if (initializedBatchesCount === null) {
			return {
				success: false,
				error: "No se pudo determinar el estado de los lotes.",
				errorCode: "DB_CHECK_INIT_NULL",
			};
		}
		if (initializedBatchesCount > 0) {
			return {
				success: false,
				error: `No se pueden eliminar los lotes porque ${initializedBatchesCount} lote(s) ya han sido inicializados.`,
				errorCode: "BATCHES_ALREADY_INITIALIZED",
			};
		}
		...
		if (idsError || !pendingBatchIdsData) {
			return {
				success: false,
				error: "Error preparando borrado (obteniendo IDs).",
				errorCode: "DB_FETCH_PENDING_IDS_ERROR",
			};
		}
		if (pendingBatchIdsData.length === 0) {
			return {
				...
				data: {
					...
					deletedItemsCount: 0,
					message: "No había lotes pendientes para eliminar.",
				},
			};
		}
		...
		const { count: itemsDeleted, error: deleteItemsError } = await supabase
			.from("article_batch_items")
			.delete({ count: "exact" })
			.in("batch_id", batchIdsToDelete);
		if (deleteItemsError) {
			return {
				success: false,
				error: `Error al eliminar artículos de los lotes: ${deleteItemsError.message}`,
				errorCode: "DB_DELETE_ITEMS_ERROR",
			};
		}
		...
		if (deleteBatchesError) {
			return {
				success: false,
				error: `Error al eliminar los lotes maestros: ${deleteBatchesError.message}`,
				errorCode: "DB_DELETE_BATCHES_ERROR",
			};
		}
		const actualBatchesDeleted = batchesDeleted || 0;

		console.log(
			`✅ [${opId}] Reseteo de lotes completado. Lotes eliminados: ${actualBatchesDeleted}, Items eliminados: ${deletedItemsCount}`
		);
		return {
			success: true,
			data: {
				deletedBatchesCount: actualBatchesDeleted,
				deletedItemsCount: deletedItemsCount,
				message: `${actualBatchesDeleted} lote(s) y ${deletedItemsCount} artículo(s) asignados fueron eliminados.`,
			},
		};
	} catch (error) {
		...
		return {
			success: false,
			error: `Error interno del servidor: ${(error as Error).message}`,
			errorCode: "INTERNAL_SERVER_ERROR_RESET",
		};
	}
}
...
export async function getProjectBatchesForDisplay(
	...
	console.log(
		`[${opId}] Iniciando getProjectBatchesForDisplay (RPC) para proyecto: ${projectId}`
	);

	if (!projectId) {
		return {
			success: false,
			error: "Se requiere ID de proyecto.",
			errorCode: "INVALID_PROJECT_ID_GPB_RPC",
		};
	}
	try {
		const supabase = await createSupabaseServerClient();
		...
		if (rpcError) {
			console.error(
				`❌ [${opId}] Error en RPC 'get_project_batches_with_assignee_names':`,
				JSON.stringify(rpcError, null, 2)
			);
			return {
				success: false,
				error: `Error al cargar lotes (RPC): ${rpcError.message}`,
				errorCode: "DB_RPC_ERROR_GPB",
			};
		}
		...
		const lotesTransformados: BatchForDisplay[] = rpcData.map(
			(lote: RpcLoteData) => ({
				id: String(lote.id),
				batch_number: Number(lote.batch_number),
				name: lote.name,
				status: lote.status as Database["public"]["Enums"]["batch_status"],
		...
		if (lotesTransformados.length > 0) {
			canResetAll = lotesTransformados.every(
				(lote) => lote.status === "pending"
			);
		}

		console.log(
			`[${opId}] Lotes obtenidos (RPC): ${lotesTransformados.length}, Puede resetear todo: ${canResetAll}`
		);
		return {
			success: true,
			data: {
				...
				totalLotes: lotesTransformados.length,
				totalLotesPending: lotesTransformados.filter(
					(l) => l.status === "pending"
				).length,
			},
		};
		...
		console.error(
			`❌ [${opId}] Excepción en getProjectBatchesForDisplay (RPC):`,
			error
		);
		return {
			success: false,
			error: `Error interno del servidor: ${(error as Error).message}`,
			errorCode: "INTERNAL_ERROR_GPB_RPC",
		};
	}
}
