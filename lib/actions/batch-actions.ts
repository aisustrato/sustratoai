...
export async function canModifyDimensionsForPhase(
	...
	console.log(
		`[${opId}] Verificando si la fase permite modificar dimensiones: ${phaseId}`,
	);
	...
	try {
		...
		const initialState =
			phaseData.phase_number === 1 ? "pending" : "translated";
		...
		if (countError) {
			return {
				success: false,
				error: "Error al verificar estado de los lotes de la fase.",
			};
		}
		...
		console.error(
			`❌ [${opId}] Excepción en canModifyDimensionsForPhase:`,
			error,
		);
		return {
			success: false,
...
async function verificarPermiso(
	...
	if (!tienePermiso)
		return { success: false, error: "No tienes permiso para gestionar lotes." };
...
export async function getBatchingStatusForActivePhase(
	...
	console.log(
		`[${opId}] Iniciando getBatchingStatus para proyecto: ${projectId}`,
	);
	...
	try {
		...
		if (quickCountError) {
			console.warn(
				`[${opId}] Error en conteo rápido, usando conteo manual:`,
				quickCountError.message,
			);
		}
		...
		if (quickCount && quickCount < 1000) {
			...
			console.log(
				`[${opId}] Conteo rápido sospechoso (${quickCount}), realizando conteo manual...`,
			);
			let totalCount = 0;
			let page = 0;
			const pageSize = 1000;
			...
			while (true) {
				...
				if (error) {
					...
					return {
						success: false,
						error: "Error al contar el universo de la fase.",
					};
				}
			...
			console.log(
				`[${opId}] Conteo manual completado: ${universeSize} artículos (${page} páginas)`,
			);
		}
		...
		if (batchCountError)
			return { success: false, error: "Error al contar lotes existentes." };
		...
		if (existingBatches === 0) {
			return {
				success: true,
				data: {
					status: "READY_FOR_BATCHING",
					activePhase: {
						id: activePhase.id,
						phase_number: activePhase.phase_number,
					},
					totalUniverseSize: universeSize,
					unbatchedArticleCount: universeSize,
				},
			};
		} else {
			const initialState: Enums<"batch_preclass_status"> =
				activePhase.phase_number === 1 ? "pending" : "translated";
			const { count: advancedBatchesCount, error: advancedCheckError } =
				await supabase
					.from("article_batches")
					.select("*", { count: "exact", head: true })
					.eq("phase_id", activePhase.id)
			...
			if (advancedCheckError)
				return {
					success: false,
					error: "Error al verificar el estado de los lotes.",
				};
			...
			return {
				...
				data: {
					...
					totalUniverseSize: universeSize,
					unbatchedArticleCount: 0,
					canResetBatches: canReset,
				},
			};
		}
	} catch (error) {
...
export async function getBatchesForPhaseDisplay(
	...
	try {
		...
		// Primero obtener el project_id de la fase
		const { data: phaseData, error: phaseError } = await supabase
			.from("preclassification_phases")
			.select("project_id")
			.eq("id", phaseId)
		...
		if (phaseError || !phaseData) {
			return {
				success: false,
				error: `Error al obtener información de la fase: ${phaseError?.message}`,
			};
		}
		...
		if (error) {
			return {
				success: false,
				error: `Error al llamar a RPC: ${error.message}`,
			};
		}

		// Filtrar solo los lotes de esta fase y mapear al formato esperado
		interface BatchFromRPC {
			id: string;
			batch_number: number;
			name: string | null;
			status: string;
		...
		const mappedData = (data || [])
			.filter((batch: unknown) => {
				const typedBatch = batch as BatchFromRPC;
				return typedBatch.phase_id === phaseId;
			})
			.map((batch: unknown) => {
				const typedBatch = batch as BatchFromRPC;
				const articleCounts = typedBatch.article_counts || {};
				const totalItems = Object.values(articleCounts).reduce(
					(sum, count) => sum + count,
					0,
				);
				...
				return {
					...
					assigned_member_name: typedBatch.assigned_to || "Sin asignar",
					phase_name: typedBatch.name || null,
					phase_id: typedBatch.phase_id,
					is_closed: typedBatch.is_closed,
					article_counts: {
						pending: articleCounts.pending || 0,
						translated: articleCounts.translated || 0,
						pending_review: articleCounts.pending_review || 0,
						reconciliation_pending: articleCounts.reconciliation_pending || 0,
						validated: articleCounts.validated || 0,
						reconciled: articleCounts.reconciled || 0,
						disputed: articleCounts.disputed || 0,
					},
				};
			});
...
export async function createBatches(
	...
	console.log(
		`[${opId}] Iniciando creación de lotes para proyecto: ${projectId}`,
	);
	...
	try {
		...
		const activePhaseResult = await getActivePhaseForProject(projectId);
		if (!activePhaseResult.data)
			return { success: false, error: "No se encontró una fase activa." };
		const activePhase = activePhaseResult.data;
		...
		if (!articleIdsResult.success) {
			return {
				success: false,
				error: `No se pudieron obtener los artículos para lotear: ${articleIdsResult.error}`,
			};
		}
		...
		if (availableArticles.length === 0)
			return {
				success: false,
				error: "No hay artículos disponibles para lotear.",
			};

		// CORRECCIÓN: Extraer solo los IDs de los objetos artículo
		const availableArticleIds = availableArticles.map(
			(article: unknown) => (article as { id: string }).id,
		);
		console.log(
			`[${opId}] Artículos disponibles: ${availableArticles.length}, IDs extraídos: ${availableArticleIds.length}`,
		);

		const articlesPerBatchArray = segmentArticles(
			availableArticleIds.length,
			batchSize,
		);
		const numBatches = articlesPerBatchArray.length;

		const { data: maxBatchNumData } = await supabase
			.from("article_batches")
			.select("batch_number")
		...
		for (let i = 0; i < numBatches; i++) {
			...
			if (batchInsertError || !newBatch)
				return {
					success: false,
					error: `Error al crear el lote ${i + 1}: ${batchInsertError?.message}`,
				};

			const articleCountForThisBatch = articlesPerBatchArray[i];
			const itemsToInsert: TablesInsert<"article_batch_items">[] =
				availableArticleIds
			...
			console.log(
				`[${opId}] Lote ${assignedBatchNumber}: insertando ${itemsToInsert.length} artículos`,
			);
			...
			if (itemsToInsert.length > 0) {
				...
				if (itemsInsertError) {
					console.error(
						`[${opId}] Error insertando artículos en lote ${assignedBatchNumber}:`,
						itemsInsertError,
					);
					return {
						success: false,
		...
		return {
			success: true,
			data: { createdBatchesCount: numBatches, totalItemsCreated },
		};
	} catch (error) {
		console.error(`❌ [${opId}] Excepción:`, error);
		return {
...
export async function resetBatchesForPhase(
	...
	ResultadoOperacion<{ deletedBatches: number; deletedItems: number }>
> {
	...
	try {
		...
		const { count: advancedCount, error: checkError } = await supabase
			.from("article_batches")
			.select("*", { count: "exact", head: true })
			.eq("phase_id", phaseId)
			.neq("status", initialState);
		...
		if (checkError)
			return { success: false, error: "Error al verificar estado de lotes." };
		if (advancedCount && advancedCount > 0) {
			return {
				success: false,
				error: `No se puede resetear, ${advancedCount} lote(s) ya están en progreso.`,
			};
		}

		const { data: batchIdsData, error: idsError } = await supabase
			.from("article_batches")
			.select("id")
			.eq("phase_id", phaseId);
		if (idsError)
			return { success: false, error: "Error obteniendo IDs de lotes." };

		const batchIdsToDelete = batchIdsData.map((b) => b.id);
		if (batchIdsToDelete.length === 0)
			return { success: true, data: { deletedBatches: 0, deletedItems: 0 } };

		const { count: deletedItems, error: itemsError } = await supabase
			.from("article_batch_items")
			.delete()
			.in("batch_id", batchIdsToDelete);
		if (itemsError)
			return { success: false, error: "Error al eliminar items de lotes." };

		const { count: deletedBatches, error: batchesError } = await supabase
			.from("article_batches")
			.delete()
			.in("id", batchIdsToDelete);
		if (batchesError)
			return { success: false, error: "Error al eliminar lotes." };

		return {
			success: true,
			data: {
				deletedBatches: deletedBatches || 0,
