    ...
    try {
        ...
        try {
            ...
            if (err instanceof Error && err.name === 'AbortError') {
                throw new Error('Timeout: Consulta a article_batches fue cancelada por tardar más de 7 segundos');
            }
            throw err;
        }
        ...
        const itemsTimeoutId = setTimeout(() => {
            console.log(`⏱️ [runTranslationJob] Abortando consulta de items por timeout`);
        ...
        try {
            ...
            clearTimeout(itemsTimeoutId);
            if (err instanceof Error && err.name === 'AbortError') {
                throw new Error('Timeout: Consulta a article_batch_items fue cancelada por tardar más de 7 segundos');
            }
            throw err;
        }
