// --- EN lib/actions/proyecto-actions.ts ---
// (Este es el archivo completo que puedes renombrar a proyecto-dashboard-actions.ts)
"use server";
    ...
    const proyectosConSettings: UserProjectSetting[] = membresias
      .map((m): UserProjectSetting | null => { 
        ...
        return {
          ...
          proposal: proyectoBase.proposal ?? null,
          proposal_bibliography: proyectoBase.proposal_bibliography ?? null,
          proposal_interviews: proyectoBase.proposal_interviews ?? null,
...
export async function obtenerProyectoPorId(
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("projects")
      .select("id, name, code, description, institution_name, status, module_bibliography, module_interviews, module_planning, proposal, proposal_bibliography, proposal_interviews")
      .eq("id", proyectoId)
      .single();
