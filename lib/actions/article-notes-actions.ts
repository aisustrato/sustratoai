...
type DetailedNoteJoined = DetailedNote & {
...
export async function getNotes(
...
): Promise<ResultadoOperacion<DetailedNote[]>> {
	console.log(
		"[SERVER] getNotes - Iniciando con filtros:",
	...
	try {
		...
		if (filters.noteId) {
			console.log(
				`[SERVER] Aplicando filtro por ID de nota: ${filters.noteId}`,
			);
			query = query.eq("id", filters.noteId);
		}
		if (filters.articleId) {
			console.log(
				`[SERVER] Aplicando filtro por ID de artículo: ${filters.articleId}`,
			);
			query = query.eq("article_id", filters.articleId);
		}
		if (filters.authorId) {
			console.log(
				`[SERVER] Aplicando filtro por ID de autor: ${filters.authorId}`,
			);
			query = query.eq("user_id", filters.authorId);
		}
		if (filters.visibility) {
			console.log(
				`[SERVER] Aplicando filtro por visibilidad: ${filters.visibility}`,
			);
			query = query.eq("visibility", filters.visibility);
		}
		if (filters.projectId) {
			console.log(
				`[SERVER] Aplicando filtro por ID de proyecto: ${filters.projectId}`,
			);
...
export async function createArticleNote(
...
): Promise<ResultadoOperacion<DetailedNote | null>> {
	console.log(
		"[SERVER] createArticleNote - Iniciando con payload:",
	...
	try {
		...
		const { data: detailedNewNote, error: fetchError } = await (supabase as any)
			.from("detailed_article_notes")
			.select("*")
			.eq("id", newNote.id)
			.single();
...
export async function updateArticleNote(
	...
	console.log(
		"[SERVER] updateArticleNote - Iniciando con payload:",
		JSON.stringify(payload, null, 2),
	);
	...
	try {
		...
		if (payload.noteContent !== undefined)
			updateData.note_content = payload.noteContent;
		...
		const { data: detailedNote, error: fetchError } = await (supabase as any)
			.from("detailed_article_notes")
			.select("*")
			.eq("id", payload.noteId)
			.single();
		...
		if (fetchError) {
			console.error(
				"[SERVER] Error al obtener la nota detallada actualizada:",
				fetchError,
			);
			throw fetchError;
		}
 ...
 */
export async function deleteArticleNote(
	noteId: string,
): Promise<ResultadoOperacion<{ id: string }>> {
	console.log(
		`[SERVER] deleteArticleNote - Iniciando eliminación de nota con ID: ${noteId}`,
	);
	...
	try {
		...
		console.log(
			`[SERVER] Eliminando nota con ID: ${noteId} para el usuario: ${user.id}`,
		);
		...
		if (error) {
			...
			const isNoRows =
				errCode === "PGRST116" || /Results contain 0 rows/i.test(errMsg);
			if (isNoRows) {
				console.warn(
					"[SERVER] Nota no encontrada o sin permisos para eliminarla",
				);
				return {
					success: false,
					error: "Nota no encontrada o sin permisos para eliminarla.",
			...
			return {
				success: false,
				error: `No se pudo eliminar la nota: ${errMsg || "Error desconocido"}`,
				errorCode: errCode || "DELETE_ERROR",
			};
		}
...
export async function getArticleNotesInfoByBatchItem(
	...
	if (
		!batchItemId ||
		typeof batchItemId !== "string" ||
		batchItemId.trim().length === 0
	) {
	...
	try {
		...
		console.log(
			`[${ts()}] [notes-rpc] Llamando RPC get_article_notes_info_by_batch_item`,
			{ batchItemId },
		);
		...
		if (!error && data) {
			...
			console.log(`[${ts()}] [notes-rpc] RPC OK`, {
				batchItemId,
				ms,
		...
		console.warn(
			`[${ts()}] [notes-rpc] RPC no disponible o sin datos. Activando fallback...`,
			{
		...
		if (abiError) {
			console.error(
				"[SERVER] Fallback: error obteniendo article_id desde article_batch_items",
				abiError,
			);
			return {
				success: false,
				error: `No se pudo obtener el artículo del ítem: ${abiError.message}`,
			};
		}

		const aid =
			(abiRow as { article_id: string | null } | null)?.article_id ?? null;
		if (!aid) {
			console.warn(
				"[SERVER] Fallback: article_id no encontrado para el ítem. Retornando vacío.",
			);
			return {
				success: true,
				data: { articleId: null, hasNotes: false, noteIds: [], noteCount: 0 },
			};
		}
		...
		if (notesError) {
			console.error(
				"[SERVER] Fallback: error consultando notas del artículo",
				notesError,
			);
			return {
				success: false,
				error: `No se pudieron obtener notas del artículo: ${notesError.message}`,
			};
		}
		...
		console.log(`[${ts()}] [notes-rpc] Fallback OK`, {
			batchItemId,
			totalMs,
			fallbackMs,
			hasNotes: fallbackResult.hasNotes,
			noteCount: fallbackResult.noteCount,
		});
