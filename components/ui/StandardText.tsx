...
type StandardTextSize = "xs" | "sm" | "base" | "md" | "lg" | "xl" | "2xl" | "3xl" | "4xl" | "5xl";
...
export function StandardText({
  ...
  if (
    typeof Component === "string" && Component.startsWith("h") && Component.length === 2 && !fontType &&
    (variant === "default" || variant === "label" || variant === "caption" || variant === "muted" || variant === "subtitle")
  ) {
    effectiveVariantForStyleDefaults = "heading";
  }

  const styleDefaults = textTokens.variants[effectiveVariantForStyleDefaults];
  let colorDefaultFromVariant = textTokens.variants[variant].color as StandardTextColorScheme;
  
  let finalColorName: StandardTextColorScheme = initialColor || colorDefaultFromVariant;
  ...
  const determineFontType = (): FontPairType => {
    ...
    if (typeof Component === "string" && Component.startsWith("h") && Component.length === 2) return "heading";
    switch (variant) {
      case "heading": case "title": case "subheading": case "subtitle": return "heading";
      default: return "body";
    }
  };
  const finalFontType = determineFontType();

  const sizeStyles: Record<StandardTextSize, string> = { xs: "text-xs", sm: "text-sm", base: "text-base", md: "text-md", lg: "text-lg", xl: "text-xl", "2xl": "text-2xl", "3xl": "text-3xl", "4xl": "text-4xl", "5xl": "text-5xl" };
  const weightStyles: Record<StandardTextWeight, string> = { normal: "font-normal", medium: "font-medium", semibold: "font-semibold", bold: "font-bold" };
  const alignStyles: Record<StandardTextAlign, string> = { left: "text-left", center: "text-center", right: "text-right", justify: "text-justify" };
  const variantStyles: Record<StandardTextVariant, string> = { default: "", heading: "tracking-tight", subheading: "", title: "", subtitle: "", label: "", caption: "", muted: "" };

  const classes = cn(variantStyles[variant], sizeStyles[finalSize], weightStyles[finalWeight], align && alignStyles[align], truncate && "truncate", className);
  ...
  if (!finalGradient) {
    return <Component className={classes} style={baseStyle} {...props}>{children}</Component>;
  }

  const gradientTypeToUse = finalGradient === true ? "primary" : finalGradient;
  const gradientColors = textTokens.gradients[gradientTypeToUse as keyof typeof textTokens.gradients];
  ...
  const gradientStyle: React.CSSProperties = {
    ...
    backgroundSize: "100%", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", color: "transparent",
    fontFamily: finalFontType === "heading" ? "var(--font-family-headings)" : "var(--font-family-base)",
    fontWeight: finalFontType === "heading" ? "var(--font-weight-headings)" : "var(--font-weight-base)",
    letterSpacing: finalFontType === "heading" ? "var(--letter-spacing-headings)" : "var(--letter-spacing-body)",
    lineHeight: "var(--line-height)", paddingBottom: "0.1em", display: "inline-block",
  };
