...
const gridContainerVariants = {
	...
	visible: {
		opacity: 1,
		transition: {
			delayChildren: 0.1, // Reducido para un inicio más rápido
			staggerChildren: 0.03, // Reducido para una secuencia más veloz
		},
	},
};

const sphereItemVariants = {
	...
	if (low > high) {
		...
		while (low <= high) {
			...
			if (totalHeightNeeded <= availableHeight) {
				const realDiameter =
					(availableWidth - GAP * (colsForSize - 1)) / colsForSize;
...
// -----------------------------------------------------------------------------
export const StandardSphereGrid = memo(
	({
		containerWidth,
		containerHeight,
		items,
		...
		const [effectiveOverflow, setEffectiveOverflow] = useState<
			"shrink" | "scroll"
		>("shrink");
		...
		useEffect(() => {
			...
			const finalVerticalMargin =
				verticalSafetyMargin ?? (hasHeader ? 100 : 20);

			const PADDING = 16;
			const RECTANGULAR_CELL_BADGE_SPACE = 24;

			const availableWidth =
				containerWidth - PADDING * 2 - finalHorizontalMargin;
			const availableHeight =
				containerHeight - PADDING * 2 - finalVerticalMargin;

			let finalLayout: LayoutCalculationResult;
			let finalAllowBadgeRender: boolean;

			if (fixedSize) {
				const canShowBadgeAtFixedSize =
					hasAnyBadge && fixedSize >= MINIMUM_SPHERE_DIAMETER_FOR_BADGE;
				const cellMode = canShowBadgeAtFixedSize ? "rectangular" : "square";
				const tempLayout = calculateLayout(
					availableWidth,
					availableHeight,
				...
				const rectangularResult = calculateLayout(
					availableWidth,
					availableHeight,
					processedItems.length,
					"rectangular",
				...
				if (
					hasAnyBadge &&
					rectangularResult.size < MINIMUM_SPHERE_DIAMETER_FOR_BADGE
				) {
		...
		const renderGridContent = useMemo(() => {
			...
			return (
				<motion.div
					key={`grid-motion-${processedItems.length}`}
					style={gridStyles}
					variants={gridContainerVariants}
					initial="hidden"
					animate="visible">
					{processedItems.map((item, index) => (
						<motion.div key={item.id} variants={sphereItemVariants}>
							<StandardSphere
								{...item}
								sizeInPx={calculatedSpherePx}
								cellHeight={calculatedCellHeight}
								allowBadgeRender={allowBadgeRender}
								colorScheme={
									item.colorScheme ||
									keyGroupColorMap[item.keyGroup as string] ||
									"primary"
								}
