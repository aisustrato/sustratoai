...
>(({ colorScheme = 'neutral', size = 'md', styleType = 'subtle', ...props }, ref) => {
  ...
  const contextValue = React.useMemo(() => ({
    ...
    mode,
    openItems,
  }), [colorScheme, size, styleType, appColorTokens, mode, openItems]);
...
>(({ className, value, disabled, children, colorScheme: itemColorScheme, styleType: itemStyleType, ...props }, ref) => {
  const { appColorTokens, mode, openItems, colorScheme, size, styleType } = useStandardAccordion();
  ...
  const tokens = React.useMemo(() => {
    ...
    return generateStandardAccordionTokens(appColorTokens, mode, { colorScheme: effectiveColorScheme, size, styleType: effectiveStyleType, isOpen, isDisabled });
  }, [appColorTokens, mode, effectiveColorScheme, size, effectiveStyleType, isOpen, isDisabled]);

  return (
    <ItemContext.Provider value={{ isOpen, isDisabled, itemColorScheme: effectiveColorScheme, itemStyleType: effectiveStyleType }}>
...
>(({ className, children, titleAlign = 'left', ...props }, ref) => {
  const { appColorTokens, mode, colorScheme, size, styleType } = useStandardAccordion();
  const { isOpen, isDisabled, itemColorScheme, itemStyleType } = useItemContext();
  ...
  const tokens = React.useMemo(() => {
    if (!appColorTokens) return null;
    const tokenArgs: StandardAccordionTokenArgs = { colorScheme: effectiveColorScheme, size, styleType: effectiveStyleType, isHovered, isOpen, isDisabled };
    return generateStandardAccordionTokens(appColorTokens, mode, tokenArgs);
  }, [appColorTokens, mode, effectiveColorScheme, effectiveStyleType, size, isHovered, isOpen, isDisabled]);
...
>(({ className, children, ...props }, ref) => {
  const { appColorTokens, mode, size, colorScheme, styleType } = useStandardAccordion();
  ...
  const tokens = React.useMemo(() => {
    if (!appColorTokens) return null;
    return generateStandardAccordionTokens(appColorTokens, mode, { colorScheme: effectiveColorScheme, size, styleType: effectiveStyleType, isDisabled });
  }, [appColorTokens, mode, effectiveColorScheme, size, effectiveStyleType, isDisabled]);
