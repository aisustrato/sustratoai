...
export function StandardSustratoLogoReactive({
	...
	const [logoColors, setLogoColors] = useState({
		primary: designTokens?.loadingLogo?.colors?.primary?.pure || "#3D7DF6",
		accent: designTokens?.loadingLogo?.colors?.accent?.pure || "#8A4EF6",
	});
	...
	// Actualizar los colores cuando cambien los tokens
	useEffect(() => {
		if (designTokens?.loadingLogo?.colors) {
			setLogoColors({
				primary: designTokens.loadingLogo.colors.primary?.pure || "#3D7DF6",
				accent: designTokens.loadingLogo.colors.accent?.pure || "#8A4EF6",
			});
	...
	const secondaryColor =
		designTokens?.loadingLogo?.colors?.secondary?.pure || logoColors.primary;
...
export function StandardSustratoLogoWithText({
	...
	const [textColors, setTextColors] = useState({
		...
		accent: designTokens?.text?.colors?.accent?.pure || "#8A4EF6",
	});

	// Actualizar los colores cuando cambien los tokens
	useEffect(() => {
		if (designTokens?.text?.colors) {
			setTextColors({
				primary: designTokens.text.colors.primary?.pure || "#3D7DF6",
				accent: designTokens.text.colors.accent?.pure || "#8A4EF6",
			});
