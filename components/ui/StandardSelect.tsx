...
const SelectOptionItem: React.FC<{
	...
	sizeTokens: SelectSizeTokens | undefined;
	...
	(
		{
			...
			pulseBorder: _pulseBorder = false,
			// eslint-disable-next-line @typescript-eslint/no-unused-vars
			pafffMoment: _pafffMoment = false,
			// eslint-disable-next-line @typescript-eslint/no-unused-vars
			stableOptions: _stableOptions = false,
			disableRipple = false,
			...restRootProps
		},
		forwardedRef,
		...
		const currentSizeTokens = React.useMemo(
			() =>
				sizeTokens || {
					height: "2.5rem",
					fontSize: "0.875rem",
			...
			<div
				...
				<div
					...
					onMouseDown={(e) => {
						...
						if (!disabled && !readOnly && !disableRipple) {
