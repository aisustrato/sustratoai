...
const StandardButton = React.forwardRef<HTMLButtonElement, StandardButtonProps>(
	(
		...
		const handleMouseDown = React.useCallback(() => {
			if (!isEffectivelyDisabled) setIsPressed(true);
		...
		const getTextColor = React.useCallback((): string => {
			...
			if (isHovered && !disabled) {
				if (variant === "ghost" || variant === "outline" || variant === "link")
					return colorTokenSet.textShade;
			...
			if (
				...
				return colorTokenSet.ghostColor;
			if (variant === "solid") return colorTokenSet.color;
		...
		const buttonContent = loading ? (
			<>
				{" "}
				<Loader2
					className={cn("mr-2", buttonTokens?.loading.spinnerSize[initialSize])}
				/>{" "}
		...
		const customStyles = React.useMemo(() => {
			...
			if (variant === "solid") {
				styles.backgroundColor = isPressed
					? colorTokenSet.activeBackground
					: isHovered
					? colorTokenSet.hoverBackground
				...
				styles.border = `1px solid ${
					isPressed
						? colorTokenSet.activeBorder
						: isHovered
						? colorTokenSet.hoverBorder
						: colorTokenSet.outlineBorder
				}`;
