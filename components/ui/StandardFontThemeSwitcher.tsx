		...
		<div className="relative flex flex-col gap-1 w-full pl-2">
			<div className="relative w-full">
				<motion.button
					...
					style={{
						backgroundColor:
							designTokens?.button?.styles?.tertiary?.subtle?.background ||
							"#f0f0f0",
						borderColor: designTokens?.input?.styles?.neutral?.border || "#ddd",
						color: designTokens?.text?.colors?.neutral?.text || "#666",
						padding: "8px 12px",
						minWidth: "180px",
						display: "flex",
						alignItems: "center",
					...
					<ChevronDown
						...
						style={{
							color:
								designTokens?.icon?.styles?.neutral?.solid?.text?.fill ||
								"#666",
				...
				<AnimatePresence>
					{isOpen && (
						<motion.div
							...
							style={{
								backgroundColor:
									(fontTokens as FontSelectorTokens | undefined)?.dropdown
										?.backgroundColor || "#fff",
								borderColor:
									(fontTokens as FontSelectorTokens | undefined)?.dropdown
										?.borderColor || "#ddd",
								...
								{fontStyles.map((font, index) => (
									<motion.button
										...
										style={{
											backgroundColor:
												fontTheme === font.id ?
													designTokens?.card?.styles?.primary?.subtle
														?.background || "#f0f0ff"
												:	"transparent",
											border:
												fontTheme === font.id ?
													`1px solid ${designTokens?.card?.styles?.primary?.subtle?.border || "#ccc"}`
										...
										<p
											style={{
												...
												color:
													fontTheme === font.id ?
														designTokens?.text?.colors?.primary?.pure ||
														"#8A4EF6"
													:	designTokens?.text?.colors?.neutral?.text || "#333",
												fontSize: "0.8rem",
												marginBottom: "0.2rem",
										...
										<p
											style={{
												...font.bodyStyle,
												color:
													designTokens?.text?.colors?.neutral?.textShade ||
													"#666",
												fontSize: "0.75rem",
												lineHeight: "1.2",
											}}>
