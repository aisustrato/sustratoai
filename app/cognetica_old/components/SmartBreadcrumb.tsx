...
export function SmartBreadcrumb() {
    ...
    const getPageLabel = (path: string): string => {
        if (path === '/cognetica_old') return 'Raíz';
        if (path === '/cognetica_old/nuevo') return 'Nuevo Artefacto';
        if (path.startsWith('/cognetica_old/') && path.split('/').length === 3) {
    ...
    if (history.length <= 1 || pathname === '/cognetica_old') {
        return null;
    }
