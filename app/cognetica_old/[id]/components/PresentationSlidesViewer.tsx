...
import {
	listArtifactPages,
	getPagePdfUrl,
} from "@/lib/actions/cognetica-old-presentation-actions";
...
export function PresentationSlidesViewer({
	...
	const handleExportPdf = async () => {
		...
		try {
			const response = await fetch(
				`/api/cognetica_old/export-pdf?artifactId=${artifactId}`,
			);
	...
	const handleExportImages = async () => {
		...
		try {
			const response = await fetch(
				`/api/cognetica_old/export-images?artifactId=${artifactId}`,
			);
