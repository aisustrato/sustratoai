...
import { deleteDistilledEssay, deleteCognitiveElements } from "@/lib/actions/cognetica-old-distillation-actions";
import { deleteLightweightElements } from "@/lib/actions/cognetica-old-lightweight-extraction-actions";
import { saveManualEssayEdit, getEssayVersionHistory, type EssayVersionHistoryItem } from "@/lib/actions/cognetica-old-essay-edit-actions";
...
export function DistilledEssayPanel({ artifactId, artifactTitle }: DistilledEssayPanelProps) {
    ...
    const loadEssay = async () => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`/api/cognetica_old/distilled-essay?artifactId=${artifactId}`);
    ...
    const handleExtractLightweight = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/cognetica_old/lightweight-extraction', {
                method: 'POST',
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/cognetica_old/distilled-essay', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ artifactId })
            });
