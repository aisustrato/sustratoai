...
/**
 * Dispara el workflow de traducción (versión Vercel Workflows, ruta
 * paralela al `startBatchTranslation` de `preclassification-actions.ts`,
 * que no se toca). Ver
 * docs/preclasificacion-auditoria-funcional/07_Requerimiento_Preclasificacion_Workflow_Vercel.md
 *
 * Route Handler (no Server Action) porque es el patrón documentado por el
 * SDK de Vercel Workflows para disparar un run.
 */
...
import { translationWorkflow, DEEPSEEK_MODEL } from "@/workflows/translation-workflow";
//#endregion ![head]

//#region [def] - 🎯 CONSTANTES 🎯
// job_type reutilizado del enum existente, sin uso previo en el código —
// distingue los runs de este workflow de los del flujo viejo ('TRANSLATION').
const WORKFLOW_JOB_TYPE = "batch_processing" as const;
//#endregion ![def]
...
export async function POST(request: Request) {
	...
	const { data: job, error: jobError } = await db
		...
		.insert({
			...
			status: "running",
			description: `[Workflow] Traduciendo Lote #${batch.batch_number}`,
			progress: 0,
			details: {
				batchId,
				total: 0,
				processed: 0,
				step: "Iniciando traducción (workflow)...",
			},
		})
		.select("id")
		.single();
	...
	if (otherJobs && otherJobs.length > 0) {
		...
		return NextResponse.json(
			{
				success: false,
				error: `Ya existe un workflow de traducción en curso para el Lote #${batch.batch_number}.`,
			},
			{ status: 409 },
		);
	}

	await start(translationWorkflow, [jobUUID, batchId, user.id]);
