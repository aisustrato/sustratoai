...
export async function POST(request: Request) {
	try {
		...
		if (
			...
			!versionType ||
			!highlightedContent ||
			!Array.isArray(highlightsMetadata)
		) {
		...
		const row: Database["public"]["Tables"]["article_abstract_highlights"]["Insert"] =
			{
				...
				highlights_metadata:
					highlightsMetadata as unknown as Database["public"]["Tables"]["article_abstract_highlights"]["Row"]["highlights_metadata"],
			};

		const { data, error } = await supabase
			.from("article_abstract_highlights")
			.upsert(row as any, { onConflict: "article_id,user_id,version_type" })
			.select()
			.single();
