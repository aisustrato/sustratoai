...
export default function BatchSimulatorPage({
	...
	const [cachedEligibleArticles, setCachedEligibleArticles] = useState<
		unknown[]
	>([]);
	...
	useEffect(() => {
		const checkPhaseAndArticles = async () => {
			...
			try {
				...
				const activePhaseResult = await getActivePhaseForProject(
					proyectoActual.id,
				);
	...
	useEffect(() => {
		const loadInitialData = async () => {
			...
			try {
				console.log(
					`📥 [CARGA INICIAL] Consultando datos una sola vez - ${new Date().toISOString()}`,
				);

				// Cargar miembros del proyecto
				const membersResult = await obtenerMiembrosConPerfilesYRolesDelProyecto(
					proyectoActual.id,
				);
				...
				console.log(
					`🔍 [CACHE] Consultando artículos elegibles para phaseId: ${activePhaseId} - ${new Date().toISOString()}`,
				);
				const articlesResult =
					await listEligibleArticlesForPhase(activePhaseId);
				...
				if (articlesResult.success && articlesResult.data) {
					...
					console.log(
						`💾 [CACHE] Artículos cacheados: ${articlesResult.data.length} - ${new Date().toISOString()}`,
					);
				} else {
					setUiError(
	...
	const handleSliderChange = (value: number[]) => {
		...
		debounceTimerRef.current = setTimeout(() => {
			console.log(
				`⏰ [DEBOUNCE] Disparando simulación tras 500ms - ${new Date().toISOString()}`,
			);
	...
	const handleSimulate = useCallback(async () => {
		...
		console.log(
			`🚀 [SIMULACIÓN] Iniciando con batchSize: ${batchSize} - ${timestamp}`,
		);
		...
		try {
			...
			console.log(
				`💾 [CACHE REUTILIZADO] Usando artículos cacheados: ${cachedEligibleArticles.length} - ${timestamp2}`,
			);
			...
			console.log(
				`⚡ [EFICIENCIA] Simulación sin consulta adicional: ${totalArticles} artículos - ${timestamp2}`,
			);
			...
			const articlesPerBatch = Array.from(
				{ length: numBatches },
			...
			console.log(
				`✅ [SIMULACIÓN COMPLETA] Artículos: ${totalArticles} | Lotes: ${numBatches} | Art/Lote: ${batchSize} - ${timestamp4}`,
			);
	...
	useEffect(() => {
		if (
			...
			console.log(
				`🚀 [TRIGGER] Ejecutando simulación - ${new Date().toISOString()}`,
			);
			handleSimulate();
	...
	useEffect(() => {
		if (
			...
			console.log(
				`🎆 [INICIAL] Simulación inicial - ${new Date().toISOString()}`,
			);
			setShouldSimulate(true);
		}
	...
	const sphereData = useMemo((): SphereItemData[] => {
		...
		simulationData.articlesPerBatch.forEach(
			(batchSize: number, index: number) => {
				const memberId = selectedMemberIds[index % selectedMemberIds.length];
	...
	// 🎯 LÓGICA DE PESO VISUAL: SOLO VARÍA EL ESPACIADO
	// - El GROSOR de las barras es CONSTANTE (4px)
	// - MENOS artículos = MÁS espacio entre barras (holgado/liviano)
	// - MÁS artículos = MENOS espacio entre barras (apretado/pesado)
	// - SIEMPRE ocupar todo el alto disponible (280px)

	if (batchSize === 1) {
		// Caso especial: 1 artículo
		dynamicBarItemHeight = pesoLoteBarContainerHeight * 0.7;
		dynamicBarItemGap = 0;
	} else if (batchSize > 1) {
		// 🔧 GROSOR CONSTANTE: Todas las barras tienen el mismo grosor
		const FIXED_BAR_HEIGHT = 4;

		// Calcular espacio total ocupado por las barras
		const totalBarsSpace = FIXED_BAR_HEIGHT * batchSize;

		// Espacio restante para gaps (debe distribuirse entre batchSize - 1 gaps)
		const totalGapSpace = pesoLoteBarContainerHeight - totalBarsSpace;

		// Espaciado entre barras (inversamente proporcional al número de artículos)
		dynamicBarItemGap = batchSize > 1 ? totalGapSpace / (batchSize - 1) : 0;
		dynamicBarItemHeight = FIXED_BAR_HEIGHT;
	} else {
		...
		<StandardPageBackground variant="gradient">
			<div className="container mx-auto py-8">
				...
				<StandardCard
					...
					<StandardCard.Content
						className={`grid md:grid-cols-2 gap-6 ${isSimulating || isCreating ? "opacity-60 pointer-events-none" : ""}`}>
						<StandardCard
							colorScheme="primary"
						...
						<StandardCard
							...
							<div className="flex gap-2 flex-wrap min-h-[40px]">
								{projectMembers.map((member) => {
									...
									const colorScheme =
										memberColorMap[member.user_id] || "neutral";
										...
										<StandardButton
											...
											onClick={() => {
												setSelectedMemberIds((prev) => {
													...
													if (currentlySelected && prev.length === 1)
														return prev;
				...
				{((simulationData && totalBatchesCalculated > 0) ||
					isSimulating ||
					isCreating) &&
					!uiError && (
						<StandardCard
							...
							<StandardCard.Content
								className={`grid md:grid-cols-3 gap-6 items-start ${isCreating ? "opacity-30 blur-sm" : ""}`}>
								<div
								...
								<div className="min-h-[300px]">
									...
									<div className="flex flex-col h-full justify-center items-center gap-4 pt-4 md:pt-0">
										<div
											style={{
												width: 120,
												height: pesoLoteBarContainerHeight,
												background:
													mode === "dark" ?
														appColorTokens.neutral.bgShade
													:	"#fff",
												borderRadius: 16,
												border: `2.5px solid ${appColorTokens.neutral.pureShade}`,
												padding: "8px",
												display: "flex",
											...
											{Array.from({ length: Math.min(batchSize, 200) }).map(
												(_, i) => (
													<div
														key={`peso-bar-${i}`}
														style={{
															width: "85%",
															height: `${dynamicBarItemHeight}px`,
															background: appColorTokens.primary.pure,
															borderRadius: "2px",
															marginTop:
																i === Math.min(batchSize, 200) - 1 ?
																	0
																:	`${dynamicBarItemGap}px`,
														}}
													/>
												),
											)}
											...
											<div className="mt-2 text-xs text-muted-foreground">
												...
												<ul className="list-disc list-inside pl-4">
													{projectMembers
														...
														.map((m) => (
															<li key={m.user_id}>
																{m.profile?.public_display_name ||
																	m.user_id.substring(0, 6)}
																:{" "}
																{simulationData.articlesPerMember?.[
																	m.user_id
																] || 0}{" "}
