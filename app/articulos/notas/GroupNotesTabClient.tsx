...
export default function GroupNotesTabClient({
	groups,
	onNoteDirtyChange,
	resetSignal = 0,
	...
	const filteredGroups: (GroupWithNotes & { filteredNotes: DetailedNote[] })[] =
		React.useMemo(() => {
			...
			return groups
				.map((g) => {
					const filteredNotes: DetailedNote[] = [
	...
	return (
		<div className="flex flex-col gap-4">
			...
			:	<StandardAccordion
					...
					className="w-full">
					{filteredGroups.map(
						(group: GroupWithNotes & { filteredNotes: DetailedNote[] }) => {
							const notes: DetailedNote[] = group.filteredNotes;
							return (
										...
										<div className="grid grid-cols-1 gap-3">
											{notes.map((note) => (
												<StandardCardWithContent
													...
													onDirtyChange={(dirty) =>
														onNoteDirtyChange?.(String((note as any).id), dirty)
													}
