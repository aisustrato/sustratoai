...
import GroupNotesTabClient, {
	type GroupWithNotes,
} from "./GroupNotesTabClient";
...
export default function NotesTabsClient({
	notasPrivadas,
	notasPublicas,
	groups,
	defaultValue = "privadas",
	autoOpenNoteId,
	autoOpenArticleId,
	...
	const [pendingValue, setPendingValue] = React.useState<
		"privadas" | "publicas" | "grupos" | null
	>(null);
	const [showUnsavedDialog, setShowUnsavedDialog] = React.useState(false);

	// Dirty tracking por nota y por tab
	const [dirtyPrivadasMap, setDirtyPrivadasMap] = React.useState<
		Record<string, boolean>
	>({});
	const [dirtyPublicasMap, setDirtyPublicasMap] = React.useState<
		Record<string, boolean>
	>({});
	const anyDirtyPrivadas = React.useMemo(
		() => Object.values(dirtyPrivadasMap).some(Boolean),
		[dirtyPrivadasMap],
	);
	const anyDirtyPublicas = React.useMemo(
		() => Object.values(dirtyPublicasMap).some(Boolean),
		[dirtyPublicasMap],
	);
	// Dirty tracking para pestaña de grupos (track por noteId)
	const [dirtyGruposMap, setDirtyGruposMap] = React.useState<
		Record<string, boolean>
	>({});
	const anyDirtyGrupos = React.useMemo(
		() => Object.values(dirtyGruposMap).some(Boolean),
		[dirtyGruposMap],
	);
	...
	const handleTabChange = (next: string) => {
		const nextValue = (
			next === "publicas" ? "publicas"
			: next === "grupos" ? "grupos"
			: "privadas") as "privadas" | "publicas" | "grupos";
		const currentDirty =
			value === "privadas" ? anyDirtyPrivadas
			: value === "publicas" ? anyDirtyPublicas
			: anyDirtyGrupos;
		if (currentDirty) {
	...
	const onDirtyChangeFactory =
		(tab: "privadas" | "publicas", id?: string) => (dirty: boolean) => {
			if (!id) return;
			if (tab === "privadas") {
					...
					:	<div className="grid grid-cols-1 gap-3">
							{notasPrivadas.map((n) => (
								<StandardCardWithContent
									...
									onDirtyChange={onDirtyChangeFactory(
										"privadas",
										String((n as any).id),
									)}
									...
									autoOpen={
										(autoOpenVisibility ?
											autoOpenVisibility === "private"
										:	true) &&
										((autoOpenNoteId ?
											String((n as any).id) === String(autoOpenNoteId)
										:	false) ||
											(autoOpenArticleId ?
												String((n as any).article_id) ===
												String(autoOpenArticleId)
											:	false))
									}
