...
export default async function ArticleNotesPage({
	...
	const [privadasRes, publicasRes] = await Promise.all([
		getNotes({
			...
			authorId: user.id,
			visibility: privateVis,
		}),
		getNotes({ projectId: proyectoActivo.id, visibility: publicVis }),
	]);

	const notasPrivadas: DetailedNote[] =
		privadasRes.success ? privadasRes.data : [];
	const notasPublicas: DetailedNote[] =
		publicasRes.success ? publicasRes.data : [];
	...
	const userGroupsInProject: GroupWithArticleCount[] =
		userGroupsRes.success ?
			userGroupsRes.data.filter((g) => g.project_id === proyectoActivo.id)
		:	[];
	const publicGroupsInProject: GroupWithArticleCount[] =
		allGroupsRes.success ?
	...
	const groupDetailsResults = await Promise.all(
		projectGroups.map(async (g) => ({
			id: String(g.id),
			base: g,
			res: await getGroupDetails(String(g.id)),
		})),
	);

	const groupsWithNotes = groupDetailsResults.reduce(
		(acc, it) => {
			if (!it.res.success || !it.res.data) return acc;
			const items = it.res.data.items as { article_id: string | null }[];
			const notesPrivate = items.flatMap((i) =>
				i.article_id ? (privateByArticle.get(i.article_id) ?? []) : [],
			);
			const notesPublic = items.flatMap((i) =>
				i.article_id ? (publicByArticle.get(i.article_id) ?? []) : [],
			);
	...
	const noteIdParam =
		typeof searchParams.noteId === "string" ? searchParams.noteId : undefined;
	const articleIdParam =
		typeof searchParams.articleId === "string" ?
			searchParams.articleId
		:	undefined;
	const modeParam =
		(
			searchParams.mode === "editor" ||
			searchParams.mode === "divided" ||
	...
	if (visibilityParam) {
		...
		const existsInPrivate = notasPrivadasEnriched.some(
			(n) => String((n as any).id) === String(noteIdParam),
		);
		const existsInPublic = notasPublicasEnriched.some(
			(n) => String((n as any).id) === String(noteIdParam),
		);
		if (existsInPrivate) {
		...
		const existsInPrivate = notasPrivadasEnriched.some(
			(n) => String((n as any).article_id) === String(articleIdParam),
		);
		const existsInPublic = notasPublicasEnriched.some(
			(n) => String((n as any).article_id) === String(articleIdParam),
		);
