...
export default async function ArticleDetailPage({
	...
	const article = res.data.article as
		| Database["public"]["Tables"]["articles"]["Row"]
		| null;
	const translations = res.data
		.translations as Database["public"]["Tables"]["article_translations"]["Row"][];
	...
	const notesArray = (notesRes.success ? notesRes.data : []) as any[];
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	const firstNote: any = notesArray.length > 0 ? notesArray[0] : null;
	const noteId: string | null = firstNote?.id ?? null;
	const noteHref =
		noteId ?
			`/articulos/notas?noteId=${noteId}&mode=editor`
		:	`/articulos/notas?articleId=${articleId}&mode=editor&visibility=private`;
	const hasNotesForArticle = !!noteId;
	const groups: GroupWithArticleCount[] =
		groupsRes.success ? groupsRes.data : [];
	...
	return (
		<div className="p-4 sm:p-6 space-y-4">
			...
			<StandardAccordion
				defaultValue="preclassification"
				type="single"
				collapsible
				colorScheme="neutral"
				styleType="subtle">
						...
						<StandardAccordionContent>
							<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
								{groups.map((g: GroupWithArticleCount) => (
									<a
										key={String(g.id)}
										href={`/articulos/grupos?groupId=${g.id}`}
										className="block">
										<StandardCard
