...
export default function ArticlePreclassificationSummary({
	summaries,
}: {
	...
	return (
		<div className="space-y-3">
			{summaries
				.sort(
					(a, b) => (a.phase.phase_number ?? 0) - (b.phase.phase_number ?? 0),
				)
				.map((s) => {
								...
								<div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
									{s.dimensions.map((dim) => {
										...
										return (
											<div
												key={dim.id}
												className="rounded-md border border-black/5 dark:border-white/10 p-3">
												<DimensionDisplay
													variant="card"
