...
export const DimensionDisplay: React.FC<DimensionDisplayProps> = ({
	...
	const resolveEmoticon = (
		...
		const entry = Object.entries(map).find(
			([k]) => k.trim().replace(/\s+/g, " ").toLowerCase() === lower,
		);
	...
	// Resolver componente de icono de lucide usando el registro ligero (sin wildcard import)
	const IconComponent =
		dimensionIcon ?
			(
	...
	return (
		<div
			className={
				variant === "card" ? "p-2" : (
					"mt-2 p-2 border-t border-gray-200 dark:border-gray-700"
				)
			}>
				...
				{review && review.value ?
					confidenceInfo ?
						<div className="flex items-center gap-2">
							{emoticon ?
								<div
									className="w-6 min-w-[1.25rem] flex items-center justify-center shrink-0"
									aria-hidden>
				...
				<StandardTooltip
					...
					trigger={
						<div>
							<StandardText
								size="xs"
								className="italic text-gray-500 line-clamp-3">
								{review.rationale}
							</StandardText>
						</div>
					}
