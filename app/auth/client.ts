...
// ========================================================================
// 🔧 createBrowserClient maneja automáticamente las cookies con @supabase/ssr
// No se necesita configuración manual - el adaptador de cookies está integrado
...
export async function signInWithEmail(
	...
	try {
		...
		if (error) {
			console.error(
				`${LOG_PREFIX_CLIENT} Error en signInWithPassword:`,
				error.message,
			);
			return {
				data: null,
		...
		console.log(
			`${LOG_PREFIX_CLIENT} Session establecida:`,
			data.session ? "SÍ" : "NO",
		);
		...
		setTimeout(async () => {
			...
			console.log(
				`${LOG_PREFIX_CLIENT} Verificación post-login - Sesión recuperada:`,
				session ? "SÍ" : "NO",
			);
			if (session) {
				console.log(
					`${LOG_PREFIX_CLIENT} User ID recuperado:`,
					session.user.id,
				);
			}
		}, 100);
		...
		console.error(
			`${LOG_PREFIX_CLIENT} Error inesperado en signInWithEmail:`,
			errorMessage,
		);
		return {
			data: null,
			error: { message: "Error inesperado al iniciar sesión", status: 500 },
		};
	}
}
...
export async function signOut(): Promise<SignOutResponse> {
	try {
		...
		const errorMessage =
			error instanceof Error ? error.message : "Error desconocido";
		console.error(
			`${LOG_PREFIX_CLIENT} Error inesperado en signOut:`,
			errorMessage,
		);
...
export async function signUp(
	...
	try {
		...
		const errorMessage =
			error instanceof Error ? error.message : "Error desconocido";
		console.error(
			`${LOG_PREFIX_CLIENT} Error inesperado en signUp:`,
			errorMessage,
		);
		return {
			data: null,
			error: { message: "Error inesperado al registrar usuario", status: 500 },
		};
	}
}
...
export async function getSession(): Promise<{
	...
	if (error) {
		console.error(
			`${LOG_PREFIX_CLIENT} Error en supabase.auth.getSession:`,
			error.message,
		);
	}
