// 📍 Selector de artefactos de Cognética
'use client';
...
export function ArtifactSelector({ 
  ...
  console.log('🎯 [ArtifactSelector] Montado con projectId:', projectId);

  useEffect(() => {
    if (!projectId) {
      console.error('❌ [ArtifactSelector] projectId es undefined!');
      setLoading(false);
      return;
    }
    loadArtifacts();
  ...
  const loadArtifacts = async () => {
    try {
      setLoading(true);
      console.log('🔍 [ArtifactSelector] Cargando artefactos para:', projectId);
      
      const response = await fetch(`/api/cognetica/artifacts?projectId=${projectId}`);
      console.log('📡 [ArtifactSelector] Response status:', response.status);
      
      if (!response.ok) {
        const errorData = await response.json();
        console.error('❌ [ArtifactSelector] Error:', errorData);
        throw new Error(errorData.error || 'Error al cargar');
      }

      const data = await response.json();
      console.log('✅ [ArtifactSelector] Recibidos:', data.count, 'artefactos');
      
      setArtifacts(data.artifacts || []);
    } catch (error) {
      console.error('❌ [ArtifactSelector] Error:', error);
      toast({
        title: 'Error',
  ...
  return (
    <StandardCard className="p-6 space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold">Seleccionar Artefactos</h3>
          <p className="text-sm text-muted-foreground">
            Conecta artefactos de Cognética como fuentes
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            <p className="text-muted-foreground">
              {searchQuery ? '🔍 No se encontraron artefactos' : '📦 No hay artefactos'}
            </p>
