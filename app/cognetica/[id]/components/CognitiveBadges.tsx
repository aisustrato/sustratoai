...
import { addSeedToArtifact } from "@/lib/actions/cognetica-actions";
...
export function CognitiveBadges({
	...
	const handleSeedClick = (content: string) => {
		router.push(`/cognetica?seed=${encodeURIComponent(content)}`);
	};

	const handleDisciplineClick = (id: string) => {
		router.push(`/cognetica?discipline=${id}`);
	};

	const handleTheoryClick = (id: string) => {
		router.push(`/cognetica?theory=${id}`);
	};

	const handleThinkerClick = (id: string) => {
		router.push(`/cognetica?thinker=${id}`);
	};
