...
function shouldIgnorePathForSession(pathname: string): boolean {
  ...
  if (
    pathname.match(
      /\.(ico|png|jpg|jpeg|svg|json|webmanifest|txt|xml|css|js|map)$/i,
    )
  )
    return true;
  return false;
...
export async function middleware(request: NextRequest) {
  ...
  if (
    (search.includes("error_code=otp_expired") || search.includes("expired")) &&
    pathname !== "/login"
  ) {
    if (isDev)
      console.log(`${LOG_PREFIX_MW}:${requestId} 🚨 Enlace expirado → /login`);
  ...
  if (shouldIgnorePathForSession(pathname)) {
    if (isDev && pathname.startsWith("/papers")) {
      console.log(
        `${LOG_PREFIX_MW}:${requestId} ✅ /papers es ruta pública, permitiendo acceso sin sesión`,
      );
    }
    return response;
  }
  ...
  const supabase = createServerClient<Database>(supabaseUrl, supabaseAnonKey, {
    cookies: {
      get(name: string) {
        ...
        if (
          isDev &&
          name === "sb-vgnteswwvallupuanfiz-auth-token" &&
          !cookieValue
        ) {
          console.log(
            `${LOG_PREFIX_MW}:${requestId} ⚠️ Cookie principal no encontrada`,
          );
        }
        return cookieValue;
      },
  ...
  try {
    ...
    if (sessionError && isDev) {
      console.error(
        `${LOG_PREFIX_MW}:${requestId} ❌ Error en getSession():`,
    ...
    if (!session) {
      if (isDev)
        console.log(`${LOG_PREFIX_MW}:${requestId} Sin sesión → /login`);
      const loginUrl = new URL("/login", request.url);
    ...
    if (shouldLog) {
      console.log(
        `${LOG_PREFIX_MW}:${requestId} ✅ Sesión válida: ${session.user?.id?.substring(0, 8)}`,
      );
    }
    return response;
  } catch (error) {
