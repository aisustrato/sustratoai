...
>(({ className, ...props }, ref) => (
	<DialogPrimitive.Overlay
		ref={ref}
		className={cn(
			"dialog-overlay fixed inset-0 z-[3000] bg-black/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 pointer-events-auto",
			className,
		)}
		{...props}
	/>
...
>(
	(
		{ className, children, colorScheme = "neutral", size = "md", ...props },
		ref,
	) => {
		...
		const cssVariables = React.useMemo<React.CSSProperties>(() => {
			if (!tokens) return {};
			const tokenSet = tokens.dialog[colorScheme] || tokens.dialog.neutral;
			...
			const vars: React.CSSProperties & {
				...
				"--dialog-close-bg-hover":
					tokenSet.close.backgroundHover || "transparent",
			};
			return vars;
		}, [tokens, colorScheme]);
		...
		return (
			<StandardDialogPortal>
				...
				<DialogPrimitive.Content
					...
					className={cn(
						"dialog-modal fixed left-1/2 top-1/2 z-[3010] w-auto h-auto min-w-[320px] -translate-x-1/2 -translate-y-1/2 border-0 p-0 shadow-xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:rounded-lg overflow-hidden",
						"grid grid-rows-[auto_1fr_auto]", // Grid que se contrae automáticamente
						// Tamaños del dialog (eliminado max-w-md hardcoded)
						{
							"max-w-sm max-h-[80vh]": size === "sm",
							"max-w-md max-h-[80vh]": size === "md",
							"max-w-lg max-h-[85vh]": size === "lg",
							"max-w-xl max-h-[85vh]": size === "xl",
							"max-w-2xl max-h-[90vh]": size === "2xl",
							"max-w-3xl max-h-[90vh]": size === "3xl",
							"max-w-4xl max-h-[92vh]": size === "4xl",
							"max-w-5xl max-h-[92vh]": size === "5xl",
							"w-[95vw] max-w-[95vw] h-[95vh] max-h-[95vh]": size === "full",
						},
...
const StandardDialogHeader = ({
	...
	<div
		className={cn(
			"flex flex-col space-y-1.5 p-6 pb-4 text-center sm:text-left border-b",
...
const StandardDialogTitle = ({
	className,
	...props
}: React.ComponentPropsWithoutRef<typeof StandardText>) => (
...
const StandardDialogDescription = ({
	className,
	...props
}: React.ComponentPropsWithoutRef<typeof StandardText>) => (
