...
function sanitizeMarkdown(markdown: string): string {
    ...
    if (codeBlockMatches && codeBlockMatches.length % 2 !== 0) {
        console.warn('⚠️ [Sanitizer] Bloques de código mal cerrados detectados, corrigiendo...');
    ...
    const cleanedLines = lines.map(line => {
        ...
        if (asteriskCount > 0 && asteriskCount % 2 !== 0 && !line.trim().startsWith('*')) {
    ...
    if (headersWithMultipleSpaces) {
        console.warn('⚠️ [Sanitizer] Headers con espacios múltiples detectados:', headersWithMultipleSpaces.length);
        headersWithMultipleSpaces.forEach(h => console.log('   📝', h.substring(0, 50)));
    }
    ...
    const originalAsterisks = sanitized;
    ...
    if (h1Matches) {
        corrections.push(`Asteriscos antes de comillas+em-dash: ${h1Matches.length}`);
    }
    ...
    if (h2Matches) {
        corrections.push(`Asteriscos antes de comillas al final de línea: ${h2Matches.length}`);
    }
    ...
    sanitized = sanitized.replace(/\*\\\*/g, '**');
    if (h3Matches) {
        corrections.push(`Patrones corruptos *\\* corregidos a **: ${h3Matches.length}`);
    }
    ...
    const brokenLines = linesArray.map(line => {
        if (line.length > 500 && !line.trim().startsWith('#') && !line.trim().startsWith('```')) {
            ...
            console.log(`⚠️ [Sanitizer] Línea larga rota: ${line.length} → ${result.split('\n').length} líneas`);
            return result;
        }
        return line;
    });
...
function preprocessMarkdownWithHeaders(markdown: string): string {
    ...
    if (hasHeaders) {
        console.log('✅ [Preprocessor] Markdown ya tiene headers, no se requiere pre-procesamiento');
        return markdown;
    }
    
    console.log('⚠️ [Preprocessor] Markdown SIN headers detectado, generando estructura automática...');
    ...
    console.log(`✅ [Preprocessor] Estructura generada: 1 H1 + ${sectionNumber - 1} secciones H2`);
...
function parseMarkdownToSections(markdown: string): MarkdownSection[] {
    ...
    let currentSection: { level: number; title: string; id: string } | null = null;
...
function renderMarkdownContent(
    ...
    const safeContent = isTruncated ? content.slice(0, MAX_CONTENT_LENGTH) : content;
    
    console.log(`📄 Renderizando contenido: ${safeContent.length} chars${isTruncated ? ' (TRUNCADO desde ' + content.length + ')' : ''}`);
    
    if (content.length > 50000) {
        // Mostrar últimos 500 caracteres para secciones muy grandes
        const last500 = content.slice(-500);
        console.log(`🔍 Últimos 500 caracteres del contenido original:\n${last500}`);
    }
    ...
    const flushList = () => {
        if (listItems.length > 0) {
            ...
            elements.push(
                <ListTag key={`list-${elements.length}`} className="ml-6 my-3 space-y-1">
    ...
    const flushTable = () => {
        if (tableRows.length > 0) {
            const headers = tableRows[0].split('|').map((h: string) => h.trim()).filter((h: string) => h);
                        ...
                        <thead className="bg-muted/50">
                            <tr>
                                {headers.map((header: string, idx: number) => (
                                    <th key={idx} className="border border-border px-4 py-2 text-left font-semibold text-base">
                                        {header}
                                    </th>
                                ))}
                            </tr>
                            ...
                            {rows.map((row: string[], rowIdx: number) => (
                                <tr key={rowIdx} className="hover:bg-muted/30">
                                    {row.map((cell: string, cellIdx: number) => (
                                        <td key={cellIdx} className="border border-border px-4 py-2 text-base">
    ...
    lines.forEach((line, idx) => {
        ...
        if (line.startsWith('>')) {
            ...
            elements.push(
                <blockquote key={`quote-${idx}`} className="border-l-4 border-primary/30 pl-4 italic text-muted-foreground my-3">
        ...
        if (line.trim().startsWith('```')) {
            elements.push(
                <pre key={`code-${idx}`} className="bg-muted/50 p-3 rounded-md my-3 overflow-x-auto">
    ...
    if (isTruncated) {
        elements.push(
            <div key="truncated-warning" className="mt-4 p-3 bg-primary/10 border border-primary/30 rounded-md">
                ...
                <p className="text-xs text-muted-foreground">
                    💡 Esta sección es muy extensa. Por rendimiento, se muestra parcialmente.
                </p>
            </div>
        );
    }
...
function renderInlineMarkdown(text: string, options?: RenderContentOptions): React.ReactNode {
    ...
    while (remaining.length > 0) {
        ...
        if (italicAsteriskMatch) {
            parts.push(<em key={key++} className="italic">{italicAsteriskMatch[1]}</em>);
        ...
        if (italicUnderscoreMatch) {
            parts.push(<em key={key++} className="italic">{italicUnderscoreMatch[1]}</em>);
...
function MarkdownSection({ 
    ...
    useEffect(() => {
        if (forceExpanded !== undefined) {
            console.log(`🔄 forceExpanded cambió para "${section.title}" (${section.id}): ${forceExpanded}`);
            console.log(`   🔍 isExpanded actual antes del cambio: ${isExpanded}`);
            console.log(`   ⚠️ SOBRESCRIBIENDO isExpanded con forceExpanded: ${forceExpanded}`);
    ...
    useEffect(() => {
        ...
        if (isHighlighted) {
            contentRef.current.style.setProperty('background-color', 'rgba(59, 130, 246, 0.1)', 'important');
            contentRef.current.style.setProperty('border-left', '4px solid rgba(59, 130, 246, 0.6)', 'important');
            ...
            contentRef.current.style.setProperty('transition', 'all 0.3s ease', 'important');
    ...
    const levelConfig = {
        1: { colorScheme: 'primary' as const, size: 'xl' as const, gradient: true, weight: 'bold' as const },
        2: { colorScheme: 'secondary' as const, size: 'lg' as const, gradient: false, weight: 'semibold' as const },
        3: { colorScheme: 'tertiary' as const, size: 'base' as const, gradient: false, weight: 'semibold' as const },
        4: { colorScheme: 'neutral' as const, size: 'sm' as const, gradient: false, weight: 'medium' as const },
        5: { colorScheme: 'neutral' as const, size: 'sm' as const, gradient: false, weight: 'medium' as const },
        6: { colorScheme: 'neutral' as const, size: 'xs' as const, gradient: false, weight: 'normal' as const },
    };

    const config = levelConfig[section.level as keyof typeof levelConfig] || levelConfig[6];
    ...
    const handleToggle = () => {
        ...
        if (onExpand) {
            console.log(`   📢 Notificando al padre: onExpand(${section.id}, ${newState})`);
    ...
    return (
        <div className={cn(
            ...
            section.level === 2 ? "border-secondary/30 pl-4" : 
            "border-muted pl-6"
        )}>
            <button
...
export function StandardMarkdownViewer({ 
    ...
    const handleManualExpand = useCallback((sectionId: string, isExpanding: boolean = true) => {
        if (isExpanding) {
            ...
            setManuallyExpandedSections(prev => {
                ...
                console.log(`   📝 manuallyExpandedSections actualizado:`, Array.from(newSet));
                return newSet;
            });
            ...
            setExpandedSections(prev => {
                const newSet = new Set(prev);
                newSet.add(sectionId);
                console.log(`   📝 expandedSections actualizado:`, Array.from(newSet));
                return newSet;
            });
        } else {
    ...
    useEffect(() => {
        ...
        console.log(`   📊 manuallyExpandedRef.current.size: ${manuallyExpandedRef.current.size}`);
        
        if (!expandAll) {
            // Si expandAll es false, mantener solo las secciones expandidas manualmente
            console.log(`🔒 expandAll=false - Manteniendo ${manuallyExpandedRef.current.size} secciones manuales`);
    ...
    return (
        <div className={cn("space-y-2", className)}>
            ...
            {sections.map((section, index) => {
                ...
                if (section.level === 3) {
                    const parentH2 = sections.slice(0, index).reverse().find(s => s.level === 2);
