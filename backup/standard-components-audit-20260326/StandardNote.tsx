...
// ⚠️ PENDIENTE MIGRACIÓN V4: Este componente necesita tokens propios.
// COMPLEJIDAD: ALTA - Editor markdown completo con toolbar, preview, scroll sync.
// Los tokens ya están precalculados en DesignTokensProvider (generateAllNoteTokens).
// TODO: Refactorizar para usar useDesignTokens() en lugar de useTheme().
// PRIORIDAD: Media (componente funcional pero no optimizado).
...
const StandardNote = React.forwardRef<HTMLDivElement, StandardNoteProps>(
	(
		...
		const effectiveColorScheme: StandardNoteBetaVariant =
			(
				colorScheme &&
				validVariants.includes(colorScheme as StandardNoteBetaVariant)
			) ?
		...
		const [internalViewMode, setInternalViewMode] =
			React.useState<ViewMode>(viewMode);
		...
		React.useImperativeHandle(
			ref,
			() => textareaRef.current?.parentElement as HTMLDivElement,
		);
		...
		// Tokens precalculados desde note en DesignTokensProvider
		const noteTokens: StandardNoteBetaTokens | null =
			designTokens?.note || null;
		//#endregion ![sub_init]

		//#region [sub_effects] - 💡 EFFECTS 💡
		React.useEffect(() => {
			const element = textareaRef.current;
			if (element && noteTokens) {
		...
		const isValidMarkup = React.useCallback(
			(text: string, before: string, after: string): boolean => {
		...
		const insertText = React.useCallback(
			(before: string, after: string = "", placeholder: string = "") => {
				...
				if (selectedText) {
					...
					const expandedText = internalValue.substring(
						expandedStart,
						expandedEnd,
					);

					isWrapped =
						expandedText.startsWith(before) &&
						expandedText.endsWith(after) &&
				...
				if (!isWrapped) {
					...
					const simulatedContext =
						beforeContext + before + textToWrap + after + afterContext;
				...
				if (isWrapped) {
					...
					if (
						...
						unwrappedText =
							"*" + textToProcess.substring(3, textToProcess.length - 3) + "*";
					} else if (
						before === "*" &&
						after === "*" &&
						textToProcess.startsWith("***") &&
						...
						unwrappedText = textToProcess.substring(
							before.length,
							textToProcess.length - after.length,
						);
					}
		...
		const applyLineFormat = React.useCallback(
			(prefix: string, isOrdered: boolean = false) => {
				const textarea = textareaRef.current;
				if (!textarea || disabled || readOnly) return;
				...
				if (isOrdered) {
					...
					shouldRemoveFormat = affectedLines.every((line) =>
						orderedRegex.test(line),
					);
		...
		const handleH2 = React.useCallback(
			() => applyLineFormat("## "),
			[applyLineFormat],
		);
		const handleH3 = React.useCallback(
			() => applyLineFormat("### "),
			[applyLineFormat],
		);
		...
		const handleDividedMode = React.useCallback(
			() => handleViewModeChange("divided"),
			[handleViewModeChange],
		);
		const handleEditorMode = React.useCallback(
			() => handleViewModeChange("editor"),
			[handleViewModeChange],
		);
		const handlePreviewMode = React.useCallback(
					...
					<div className="flex flex-wrap items-center gap-2 p-3 border-b border-[var(--note-border)] bg-[var(--note-bg)]">
						...
						<div className="flex items-center gap-1">
							...
							<StandardButton
								...
								colorScheme="primary"
								leftIcon={
									effectiveViewMode === "divided" ? Columns2
									: effectiveViewMode === "editor" ?
										Edit3
									:	EyeOff
								}
						...
						<div className="flex flex-col h-[400px] max-h-[400px]">
							...
							<div
								...
								className={cn(
									...
									internalValue !== debouncedValue ? "opacity-75" : (
										"opacity-100"
									),
								)}>
								<MatrixLineRenderer
