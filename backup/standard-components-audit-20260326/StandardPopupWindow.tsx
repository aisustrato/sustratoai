...
interface StandardPopupWindowContentProps
...
>(
	(
		{ className, children, colorScheme = "neutral", size = "lg", ...props },
		ref,
	) => {
		...
		const cssVariables = React.useMemo<React.CSSProperties>(() => {
			...
			const tokenSet =
				designTokens.dialog[colorScheme] || designTokens.dialog.neutral;
...
const StandardPopupWindowHeader = ({
	className,
	...props
...
const StandardPopupWindowFooter = ({
	className,
	...props
}: React.HTMLAttributes<HTMLDivElement>) => (
	<div
...
const StandardPopupWindowTitle = ({
	className,
	...props
...
const StandardPopupWindowDescription = ({
	className,
	...props
}: React.ComponentPropsWithoutRef<typeof StandardText>) => (
	<StandardText
